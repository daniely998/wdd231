...
function createCourseList(courses) {
	...
	courses.forEach(course => {
        ...
        {
            subject.style.backgroundColor = "gray";
            subject.textContent = '\u2713 ';
        }
        else
        {
            subject.style.backgroundColor = "silver";
        }
        subject.textContent += course.subject + " " + course.number;

		document.querySelector(".courselist").appendChild(subject);
	});
    let totalCredit = document.createElement("p");
    const total = courses.reduce((acc, course) => acc + course.credits, 0);
    totalCredit.textContent = `The total credit is: ${total}`;
    document.querySelector(".courselist").appendChild(totalCredit);
}
